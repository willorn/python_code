...
def level_up(filename):
    new_content = []
    # 【from：】
    # https://github.com/ayasa520/markown-splitter
...
if __name__ == '__main__':
    # filename = ''
    filename = sys.argv[1]
    func = [level_up, level_down, ]
    print(filename)
    if filename[-3:] != '.md':
        cont = input("非 md 文档, 是否继续? y/n\n")
        if cont == 'n':
            exit()
    operate = input("输入要完成的操作\n1: 所有标题等级提升 1 级\n2: 所有标题等级降低 1 级\n3: 将文档按一级标题拆分开\n")
    try:
        print("正在操作:", int(operate))
        operate_type = int(operate) - 1
        func[operate_type](filename)
    except:
        print("异常退出")
        exit()
